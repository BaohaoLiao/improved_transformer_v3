...
class QuantizedBertLayer(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        ...
        self.attention = quantize_model(org_model.attention, specials=attention_specials, **quant_params)
        if self.add_cross_attention:
            self.crossattention = quantize_model(org_model.crossattention, specials=attention_specials, **quant_params)
...
class QuantizedBertPooler(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        super().__init__()
        self.dense_act = quantize_model(nn.Sequential(org_model.dense, org_model.activation), **quant_params)
...
class QuantizedBertPredictionHeadTransform(QuantizedModel):
    def __init__(self, org_model, **quant_params):
        ...
        if org_model.transform_act_fn == F.gelu:
            ...
            raise ValueError(f'transform activation fn "{org_model.transform_act_fn}" ' f"is not supported")

        self.dense_act = quantize_model(nn.Sequential(org_model.dense, transform_act_fn), **quant_params)
