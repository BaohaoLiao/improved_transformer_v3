...
def clipped_softmax(data, attention_mask: torch.FloatTensor, dim=1, alpha=1., eta=1., **kw):
    """
    ...
    gamma = (-alpha / src_len).to(dtype).to(device).unsqueeze(-1)
    mask = attention_mask == 0

    sm_out = nn.functional.softmax(data, dim=dim, **kw)
    print("before", sm_out[0], src_len, gamma)
    stretched_out = (sm_out * (eta - gamma) + gamma) * mask
    print("after", stretched_out)
    return torch.clip(stretched_out, 0, 1)


def normalized_clipped_softmax(data, attention_mask: torch.FloatTensor, dim=1, eta=1.03, beta=1.0, **kw):
    """
    attention_mask: 0 means unmask, -inf means mask
    """
    dtype, device = data.dtype, data.device
    ...
    src_len = (attention_mask == 0).sum(dim=-1)
    factor = ((src_len * eta - beta) / (src_len - 1)).to(dtype).to(device)
    factor[factor == np.inf] = 1 # for src_len = 1
    shift = ((beta - eta) / (src_len - 1)).to(dtype).to(device)
    shift[shift == -np.inf] = 0
    factor = factor.unsqueeze(-1)
    shift = shift.unsqueeze(-1)

    sm_out = nn.functional.softmax(data, dim=dim, **kw)
    mask = attention_mask == 0 # for pad token
...
class BertSelfAttentionWithExtras(nn.Module):
    def __init__(
        ...
        self.eta = eta
        self.alpha = alpha
        if self.alpha is not None: # clipped softmax
            self.softmax_fn = partial(clipped_softmax, alpha=self.alpha, eta=1.0)
        elif eta is not None: # baohao
    ...
    def forward(
        ...
        if (self.eta is not None) or (self.alpha is not None): # clipped or baohao
