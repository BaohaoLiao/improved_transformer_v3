...
class ModelArguments:
    ...
    gamma: float = field(
        default=0,
        metadata={
            "help": ("gamma value for clipping")
        },
    )
    gated_attention: bool = field(
        default=False,
        metadata={
            "help": ("apply gated attention")
        },
    )
    b_init: float = field(
        default=1,
        metadata={
            "help": ("initialized bias term for the gated linear layer")
        },
    )
...
def main():
    ...
    if model_args.config_name:
        ...
        config.gamma = model_args.gamma
        config.gated_attention = model_args.gated_attention
        config.b_init = model_args.b_init
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            tokenized_datasets = raw_datasets.map(
                ...
                desc="Running zztokenizer on dataset",
            )
        else:
            tokenized_datasets = raw_datasets.map(
                tokenize_function,
